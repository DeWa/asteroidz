...
function enemyLogic() {
    // If 3 or less asteroid on the field, add one more
    if(bAsteroids.length < 3) {
...
function drawBullets() {
    for(var i = 0; i < bullets.length; i++ ) {
        ...
        if(bullets[i].location.x > cw || bullets[i].location.x < 0 || bullets[i].location.y > ch || bullets[i].location.y < 0){
            bullets.splice(i, 1);
        }
    }
}
...
function hitDetection(){
    ...
    if(bullets.length > 0) {
        for(var i = 0; i < bullets.length; i++) {
            for(var j = 0; j < bAsteroids.length; j++) {
                if(isHit(bullets[i].hitbox, bAsteroids[j].hitbox)) {
                    // Kill asteroid
                    bAsteroids[j].die();
                    // Remove asteroid from array
                    bAsteroids.splice(j,1);
    ...
    // Player to asteroid
    for(var i = 0; i < bAsteroids.length; i++) {
        if(isHit(player.hitbox, bAsteroids[i].hitbox)) {
            // Kill asteroid
            bAsteroids[i].die();
            // Remove asteroid from array
            bAsteroids.splice(i,1);
            player.die();
        }
    }
}

// Ugly hit detection function
function isHit(hitbox1, hitbox2) {
    var dx = (hitbox1.x + hitbox1.radius) - (hitbox2.x + hitbox2.radius);
    var dy = (hitbox1.y + hitbox1.radius) - (hitbox2.y + hitbox2.radius);
...
function onKeyDown(event) {
    ...
    else if (event.keyCode == 17 || event.keyCode == 17) player.shooting = true;
    }
...
function onKeyUp(event) {
    ...
    else if (event.keyCode == 17 || event.keyCode == 17) player.shooting = false;
 }
